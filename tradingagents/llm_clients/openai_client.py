...
# After the OpenAI SDK's short inner retries exhaust, wait briefly and continue
# the same analyst step — gateway blips (new_api do_request_failed / 5xx) often
# clear in a couple of seconds. Do not bubble up to a full job requeue.
_TRANSIENT_LLM_OUTER_RETRIES = 2
_TRANSIENT_LLM_BACKOFF_SECONDS = 2.5
...
def _is_transient_llm_error(exc: BaseException) -> bool:
    """Whether ``exc`` is a short-lived upstream/gateway failure worth pausing on."""
...
def _call_with_transient_retry(func, /, *args, **kwargs):
    """Invoke ``func``, pausing briefly on transient LLM gateway errors."""
    last_error: BaseException | None = None
    for attempt in range(_TRANSIENT_LLM_OUTER_RETRIES + 1):
        try:
            ...
            if (
                ...
                or attempt >= _TRANSIENT_LLM_OUTER_RETRIES
            ):
                raise
            logger.warning(
                "LLM transient error; waiting %.1fs then continuing "
                "(attempt %s/%s): %s",
                _TRANSIENT_LLM_BACKOFF_SECONDS,
                attempt + 1,
                _TRANSIENT_LLM_OUTER_RETRIES + 1,
                exc,
            )
            time.sleep(_TRANSIENT_LLM_BACKOFF_SECONDS)
...
class NormalizedChatOpenAI(ChatOpenAI):
    ...
    def _generate(self, messages, stop=None, run_manager=None, **kwargs):
        # Cover analyst/researcher ``llm.invoke`` paths: SDK retries are short;
        # pause 2–3s here so a transient nbapi/new_api 500 does not fail the job.
