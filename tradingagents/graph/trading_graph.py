...
from tradingagents.dataflows.market_data_validator import get_verified_market_bundle
from tradingagents.dataflows.structured_data import get_ohlcv
from tradingagents.dataflows.tradingview.fundamentals import build_fundamentals_chart
...
class TradingAgentsGraph:
    ...
    def _enrich_decision_brief(final_state: dict, ticker: str, trade_date: str) -> None:
        """Attach verified prices, OHLCV snapshot, and statement-period chart data."""
        try:
            bundle = get_verified_market_bundle(ticker, trade_date)
        ...
        chart = bundle.get("price_chart") if isinstance(bundle, dict) else None
        if isinstance(chart, dict) and chart.get("bars"):
            final_state["price_chart"] = chart
        try:
            fundamentals_chart = build_fundamentals_chart(
                ticker,
                trade_date,
                payload_available_on=trade_date,
            )
        except Exception as exc:  # noqa: BLE001 - missing statements must not fail the analysis
            logger.warning(
                "Could not attach fundamentals chart for %s on %s: %s",
                ticker,
                trade_date,
                exc,
            )
            fundamentals_chart = None
        if isinstance(fundamentals_chart, dict) and fundamentals_chart.get("periods"):
            final_state["fundamentals_chart"] = fundamentals_chart
        brief = final_state.get("decision_brief")
        if not isinstance(brief, dict):
            return
        source_currency = brief.get("currency")
        verified_currency = reference.get("currency")
        if (
            brief.get("target_price") is not None
            and isinstance(source_currency, str)
            and isinstance(verified_currency, str)
            and source_currency.upper() != verified_currency.upper()
        ):
            logger.warning(
                "Discarding decision-brief target price for %s: text currency %s "
                "does not match verified quote currency %s",
                ticker,
                source_currency,
                verified_currency,
            )
            brief["target_price"] = None
            brief["target_price_rejected"] = True
    ...
    def _log_state(self, trade_date, final_state):
        ...
        self.log_states_dict[str(trade_date)] = {
            ...
            "price_chart": final_state.get("price_chart"),
